...
void send_sse(int client_fd, int* count) {
    ...
    async_write(client_fd, response, strlen(response));
}
...
int server() {
    ...
    while (RUNNING) {
        ...
        char    request[1024];
        ssize_t request_len = async_read(client_fd, request, sizeof(request));
        ...
        if (request_len > 0 && strstr(request, "GET /ooga_booga")) {
            ...
            async_write(client_fd, response_headers, strlen(response_headers));
